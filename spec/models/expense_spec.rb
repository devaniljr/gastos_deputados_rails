...
RSpec.describe Expense, type: :model do
  context 'Validations' do
    ...
    it 'is invalid without a type_of_expense' do
      ...
      expect(expense.errors[:type_of_expense]).to include("can't be blank")
    end
    ...
    expect(expense.errors[:provider_document]).to include("can't be blank")
  end
    it 'is invalid without a provider_name' do
      expense = FactoryBot.build(:expense, provider_name: nil)
      expense.valid?

      expect(expense.errors[:provider_name]).to include("can't be blank")
    end
    ...
    it 'is invalid without a document_url' do
      ...
      expect(expense.errors[:document_url]).to include("can't be blank")
    end
  end
end
